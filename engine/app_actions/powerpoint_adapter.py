...
class PowerPointAdapter:
    ...
    def undo(self, prepared, record=None):
        ...
        with exact_office_document(
            ...
            slide, shape, state = self._context(application, presentation)
            if int(state["slide_id"]) != int(prepared.current_state["slide_id"]) or int(
                state["shape_id"]
            ) != int(prepared.current_state["shape_id"]):
                raise AppActionContextChanged(
            ...
            if prepared.operation == "replace_shape_text":
                ...
                restored = (
                    full
                    if prepared.params.get("selection_type") == PPT_SELECTION_SHAPES
                ...
                current = self._style_state(target)
                ...
                self._apply_font(target, original)
                restored = self._style_state(target)
                if any(restored.get(key) != value for key, value in original.items()):
                    raise AppActionVerificationError(
                        "PowerPoint 글자 서식 복원을 확인하지 못했습니다."
                    )
            elif prepared.operation == "set_text_alignment":
                target = self._range_for_prepared(shape, prepared)
                if int(target.ParagraphFormat.Alignment) != int(
                    prepared.params["alignment"]
                ):
                    raise AppActionContextChanged(
            ...
            elif prepared.operation == "move_shape":
                shape_id = state["shape_id"]
                if not self._position_matches(
                    self._geometry_state(slide, shape_id),
                    prepared.params["left"],
            ...
            elif prepared.operation == "resize_shape":
                shape_id = state["shape_id"]
                ...
                target = self._range_for_prepared(shape, prepared)
                desired = dict(prepared.params["desired"])
                if self._style_state(target) != desired:
                    raise AppActionContextChanged(
                        "PowerPoint 스타일이 직전 편집 결과와 다릅니다."
                    )
                self._apply_style(target, prepared.params["original"])
                if self._style_state(target) != prepared.params["original"]:
                    raise AppActionVerificationError(
