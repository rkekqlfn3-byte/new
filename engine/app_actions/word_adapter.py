...
from engine.app_actions.operations.word import (
    WD_ALIGNMENTS,
    WORD_OPERATIONS,
    WordSession,
    apply_text_format,
...
# per-operation split.
__all__ = ["WD_ALIGNMENTS", "MAX_WORD_TEXT_CHARS", "WordAdapter"]


class WordAdapter:
    """COM lifecycle, document identity and the shared result shape.

    Every user-visible action lives in ``engine.app_actions.operations.word``.
    """

    supported_operations = WORD_OPERATIONS.names

    # Shared with the operations and with ``undo`` while restoring.
    _apply_text_format = staticmethod(apply_text_format)
    ...
    def prepare(self, operation, params):
        operation_module = WORD_OPERATIONS.require(operation)
        if not isinstance(params, dict):
            raise AppActionBlocked("Word 작업의 params는 객체 형식이어야 합니다.")
        with self._document_session(self._document_path(params)) as session:
            return operation_module.prepare(self, session, params)

    @contextmanager
    def _document_session(self, path):
        """Open the exact connected document once for one operation."""
        with exact_office_document(
            ...
            yield WordSession(application, document)
    ...
    def read_selection(self, document_path):
        ...
        with self._document_session(path) as session:
            _, state = self._context(session.application, session.document)
            return {
                "document_id": state["document_id"],
                "start": state["start"],
                "end": state["end"],
    ...
    def execute(self, prepared):
        ...
        operation_module = WORD_OPERATIONS.require_prepared(prepared)
