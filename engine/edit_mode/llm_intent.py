...
HWP_OPERATION_GUIDE: dict[str, tuple[str, tuple[str, ...]]] = {
    ...
    "open_hwp_dialog": (
        ...
        "dialog_action 은 CharShape(글자 모양), ParagraphShape(문단 모양), "
        "MultiColumn(단 설정), MakeIndex(색인), MakeContents(목차), "
        "SpellingCheck(맞춤법), InputHanja(한자 변환), HeaderFooter(머리말), "
        "PageNumPos(쪽 번호), Sort(정렬), InsertChart(차트), "
        "DocSummaryInfo(문서 정보), ComposeChars(글자 겹치기), "
        "ConvertCase(대소문자) 중 하나. 실행이 되는 다른 작업이 있으면 "
        "그것을 먼저 쓴다",
        ("dialog_action",),
    ),
    "run_ribbon_action": (
