...
class CommandParser:
    ...
    @staticmethod
    def _parse_native_excel_sum_command(user_input):
    ...
    @staticmethod
    def _parse_native_excel_format_command(user_input):
        text = str(user_input or "").strip()
    ...
    @staticmethod
    def _parse_native_excel_range_format_command(user_input):
        text = str(user_input or "").strip()
    ...
    @staticmethod
    def _parse_native_excel_filter_command(user_input):
        text = str(user_input or "").strip()
        if not re.match(r"^(?:엑셀|excel)(?:에서)?\s+", text, re.IGNORECASE):
            return None
        if "필터" not in text:
    ...
    @staticmethod
    def _parse_native_excel_find_replace_command(user_input):
        text = str(user_input or "").strip()
        if not re.match(r"^(?:엑셀|excel)(?:에서)?\s+", text, re.IGNORECASE):
            return None
    ...
    @staticmethod
    def _parse_native_excel_sort_command(user_input):
        text = str(user_input or "").strip()
        if not re.match(r"^(?:엑셀|excel)(?:에서)?\s+", text, re.IGNORECASE):
            return None
        if "정렬" not in text:
    ...
    @staticmethod
    def _parse_native_hwp_find_replace_command(user_input):
        text = str(user_input or "").strip()
    ...
    @staticmethod
    def _parse_native_hwp_text_format_command(user_input):
        text = str(user_input or "").strip()
        if not re.match(r"^(?:한글|hwp)(?:에서|에)?\s+", text, re.IGNORECASE):
            return None
        if "선택" not in text:
    ...
    @staticmethod
    def _parse_native_hwp_paragraph_format_command(user_input):
        text = str(user_input or "").strip()
        if not re.match(r"^(?:한글|hwp)(?:에서|에)?\s+", text, re.IGNORECASE):
            return None
    ...
    @staticmethod
    def _parse_native_hwp_insert_command(user_input):
        text = str(user_input or "").strip()
        match = re.match(
            r"^(?:한글|hwp)(?:에서|에)?\s+"
    ...
    @staticmethod
    def _parse_native_hwp_save_command(user_input):
        text = str(user_input or "").strip()
        if not re.match(r"^(?:한글|hwp)(?:에서|에)?\s+", text, re.IGNORECASE):
            return None
    ...
    def resolve_pending_confirmation(
        ...
        resolution = self.confirmation_response_handler.resolve_selection(
            session_id,
            confirmation_id=confirmation_id,
            option_id=option_id,
            user_text=user_text,
    ...
    def _parse_and_execute_core(self, user_input, log_callback=None, image_data=None, mode="command", use_api=False, summary="", stream_callback=None, conversation_state=None, session_id=None):
        self.action_executor.noun_dict = self.dict_mgr.noun_dict
        # 1. 대화/질문 모드 (Conversation/Question Mode) - AI 엔진(Ollama/API) 사용
        if mode in ["conversation", "question"]:
            latest_user_text = self._latest_user_text(user_input)
            if self._is_current_date_question(latest_user_text):
                if log_callback:
                    log_callback("[Parser] 현재 날짜 질문을 로컬 시스템 시계로 처리합니다.")
                return self.builtins.handle_date()

            if log_callback: log_callback(f"[Parser] {mode} 모드(AI) 실행 중... (history length: {len(user_input) if isinstance(user_input, list) else 1})")
            
            result = self.llm_engine.process_command(user_input, image_data, mode=mode, use_api=use_api, summary=summary, stream_callback=stream_callback, conversation_state=conversation_state)
            ...
            if action == "open_app" and target:
                path = self.dict_mgr.noun_dict.get(target)
                if path:
                    try:
            ...
            if conversation_error:
                return failure_result(
                    ...
                    error_type=self._failure_type_for_error(conversation_error),
        ...
        if getattr(self, "pending_macros", []):
            ...
            if user_input_str in approval_words:
                return self.approve_pending_learning()
            if user_input_str in rejection_words:
                return self.reject_pending_learning()
            return failure_result(
        ...
        if user_input_str in {"확인 카드 테스트", "확인카드 테스트"}:
            return self._queue_demo_confirmation(user_input_str, session_id)

        native_excel_request = (
            self._parse_native_excel_write_command(raw_user_input_str)
            or self._parse_native_excel_sum_command(raw_user_input_str)
            or self._parse_native_excel_format_command(raw_user_input_str)
            or self._parse_native_excel_range_format_command(raw_user_input_str)
            or self._parse_native_excel_filter_command(raw_user_input_str)
            or self._parse_native_excel_find_replace_command(raw_user_input_str)
            or self._parse_native_excel_sort_command(raw_user_input_str)
        )
        native_hwp_request = (
            self._parse_native_hwp_find_replace_command(raw_user_input_str)
            or self._parse_native_hwp_text_format_command(raw_user_input_str)
            or self._parse_native_hwp_paragraph_format_command(raw_user_input_str)
            or self._parse_native_hwp_insert_command(raw_user_input_str)
            or self._parse_native_hwp_save_command(raw_user_input_str)
        )
        ...
        if native_app_request:
            ...
            if native_app_request.get("operation") == "choose_hwp_replace_scope":
                return self._queue_hwp_scope_choice(
                    native_app_request,
                    session_id,
                    raw_user_input_str,
                    log_callback=log_callback,
        ...
        routing_mode = self.dict_mgr.get_ai_config().get("routing_mode", "auto")
        if routing_mode not in {"auto", "local_only", "ai_first"}:
            routing_mode = "auto"

        analysis = self.analyze_command(strip_run_directive(user_input_str))
        force_ai_fallback = (
        ...
        if force_ai_fallback:
            ...
            if (
                ...
                return self._execute_local_compound(
        ...
        normalized_tokens = analysis.get("tokens", self.normalize_text(user_input_str))
        ...
        # Execute Macro
        if matched_macro:
            macro_data = self.dict_mgr.macro_dict[matched_macro]
            ...
            if macro_type == "hotkey":
                ...
                learned_macro = getattr(self.dict_mgr, "learned_macros", {}).get(
                    app_name, {}
                ).get(matched_macro)
                ...
                target_arg = self._build_learned_argument(
                    learned_macro, template_match, matched_app_name, matched_app_path,
                )
                policy_gate = self._local_skill_policy_gate(
                    app_name=app_name,
            ...
            handler = self._handlers.get(matched_macro)
            if handler:
        ...
        result = self.llm_engine.process_command(user_input_str, image_data, mode=mode, use_api=use_api, stream_callback=stream_callback)
        ...
        return self.ai_action_handler.handle_result(
            result,
            user_input_str,
            log_callback=log_callback,
            session_id=session_id,
