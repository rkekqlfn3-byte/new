...
공식 Automation 보안 모듈 등록과 실제 파일을 먼저 읽기 전용으로 확인하므로,
현재처럼 모듈이 없으면 HWP를 시작하지 않고 `environment_error`로 즉시
안내합니다. 등록이 있으면 같은 이름으로 `RegisterModule`을 통과한 뒤에만
파일 저장을 시작합니다.
...
2026-07-18 최근 검증 산출물 편집 세션 인계 보강 기준으로 전체 자동 테스트
871개 중 868개가 통과했고(unit 334·integration 331·windows 203), 실제 외부
