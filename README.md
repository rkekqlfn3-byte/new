...
검증한 텍스트 편집 직후 같은 구조적 시작점의 문장을 사용자가 명확히 짧게 다듬은
경우도 파일 범위의 `간결한 문체` 관찰 증거가 됩니다. Word의 범위 끝점, 한글의
선택 끝 좌표, PowerPoint의 텍스트 끝점이 축약으로 달라져도 문서 fingerprint와
앱별 시작 식별자가 같으면 동일 대상으로 확인합니다. 선택 시작점 변경, 사소한
축약, 거의 전부 삭제, Excel 셀 변경은 제외하며 자유 문장과 문서 원문은 저장하지
않습니다. 직접 수정 증거 역시 같은 값 3회와 사용자 승인이 있어야 기본값으로
활성화됩니다.
...
2026-07-18 실행 전 복구 계약·직접 수정 관찰 반영 기준으로 전체 자동 테스트
729개 중 726개가 통과했고(unit 233·integration 295·windows 198), 실제 외부
