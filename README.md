...
조인하지 않습니다. `고객 시트와 주문 시트를 고객ID로 내부 조인해서 Word
보고서와 5장 PPT 만들어줘`처럼 공통 키를 말하거나, `고객 시트의 고객ID와
주문 시트의 구매자ID로 내부 조인해서 ...`처럼 양쪽 키를 각각 말하면서
두 시트·키·내부/왼쪽 방식을 모두 명시하면 정확한 계획을 미리 보여주고
승인받은 뒤 읽기 전용 파생 표를 만듭니다.
오른쪽 키가 중복된 경우에는 `... 주문 시트의 매출을 합계 집계해서 왼쪽
조인해서 ...`처럼 오른쪽 시트와 숫자 열의 합계를 명시할 수 있습니다. 왼쪽 키가
고유한 경우에만 오른쪽 키별 합계를 만든 뒤 조인합니다. 승인 없는 다대다·측정값
키·비숫자 집계·행/열 한도 초과는 추측하지 않고 차단하며, 시트명과 키는 장기
재사용 스킬에 저장하지 않습니다.
...
901개 중 898개가 통과했고(unit 361·integration 334·windows 203), 실제 외부
