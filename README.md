...
전용으로 확인하므로, 모듈이 없는 환경에서는 승인 카드·Excel 분석·영구 상태·
...
2026-07-29 기준 이 개발 PC에서는 `FilePathCheckerModuleExample` 등록과 실제
모듈 파일이 확인되었습니다. 이 상태는 실행할 때마다 다시 검사하며, 다른 PC의
미등록 상태를 성공으로 간주하지 않습니다.
...
`Word 보고서만 만들어줘` 또는 `한글 보고서를 만들고 PPT는 빼줘`라고 말하면
현재 연결 Excel 전체를 읽기 전용으로 분석해 보고서만 만들며, 승인 계획에
PowerPoint 단계나 `.pptx` 경로를 넣지 않습니다. `보고서만 만드는 법 알려줘` 같은
질문은 실행하지 않습니다.
반대로 `PPT만 5장으로 만들어줘` 또는 `보고서 없이 발표자료 6장 만들어줘`라고
말하면 보고서 단계·`.docx`·`.hwp` 경로 없이 PowerPoint만 생성합니다. 보고서와
발표자료를 모두 제외하거나 `PPT만`과 보고서 생성을 동시에 요구하면 실행 전에
다시 말해 달라고 안내합니다.
...
여러 표시 시트에 키 열이 있으면 실제 키 값은 저장하지 않은 채 일치 수와
1:1·1:N 같은 관계 후보를 보고합니다. 이름이 다른 키는 양쪽이 ID·코드·번호형이고
겹치는 키 3개 이상·양쪽 포함률 80% 이상·최소 한쪽이 거의 고유한 경우에만
검토 후보로 표시합니다. 범주 2~20개와 숫자 측정 열이 있는
...
키를 모르겠으면 `시트 관계 후보 찾아줘` 또는 `조인 키 후보 알려줘`라고 말할 수
있습니다. 같은 머리글 후보와 엄격한 기준을 통과한 이름이 다른 후보를 최대
10개까지 시트명·양쪽 키명·
1:1/1:N/N:1/N:M·겹치는 키 수로만 보여줍니다. 셀 값과 파일 경로는 저장하지 않고,
이름이 다른 후보는 항상 사용자 검토 대상으로 남기며 여러 열과 동시에 맞으면
복수 후보임을 표시합니다. 후보가 하나여도 자동 조인하지 않습니다. 실제 조인은
두 시트·양쪽 키·내부/왼쪽
방식을 다시 말하고 별도 미리보기를 승인해야 합니다.
후보를 확인한 직후에는 `1번 후보로 왼쪽 조인해서 Word 보고서와 5장 PPT
만들어줘`처럼 번호를 사용할 수도 있습니다. 후보는 현재 편집 세션 메모리에서
10분만 유지되고 파일이나 세션이 바뀌면 폐기됩니다. 번호는 시트·양쪽 키를
미리보기에 정확히 채우는 용도일 뿐이며, 승인 없이 조인을 실행하지 않습니다.
N:M 후보나 사전 집계 요청은 번호로 축약하지 않고 시트·키·집계 열과 방식을
직접 말해야 합니다.
키가 중복된 경우에는 `... 고객 시트의 구매액을 합계 집계하고 주문 시트의 매출을
합계 집계해서 왼쪽 조인해서 ...`처럼 왼쪽과 오른쪽의 시트·열·합계/평균/건수/
최솟값/최댓값을 각각 명시할 수 있습니다. 각 쪽 최대 5개를 키별로 먼저 계산하며,
사전 집계한 쪽은 키와 파생 집계 열만 결과에 포함합니다. 건수는 명시한 열의 비어
있지 않은 값만 셉니다. 승인 없는 다대다·측정값 키·비숫자가 포함된
합계/평균/최솟값/최댓값·행/열
한도 초과는 추측하지 않고 차단하며, 시트명·키·집계 계약은 장기 재사용 스킬에
저장하지 않습니다.

검증 완료한 복합 업무는 보고서 형식·발표자료 포함 여부·PPT 장수와 내용 없는 등록 단계 레시피만 재사용 후보로
...
경로를 다시 검증하고 전체 작업을 다시 승인받으며, 현재 명령에 적은 형식·발표자료 포함 여부·장수는
기억한 값보다 우선합니다.

실행 상태에는 각 단계의 선행 단계·부작용 종류·완료 증거·중복 방지 키가 내용과
경로 없이 고정됩니다. 승인 계획이나 저장 상태에서 이 계약이 바뀌면 실제 Office를
호출하기 전에 차단하고, 실패 후에는 선행 증거가 유효한 첫 미완료 단계부터만
재개합니다. 실행기와 학습 스킬은 같은 허용 단계 레지스트리를 사용하며 등록되지
않은 단계나 순서·부작용·완료 증거가 바뀐 레시피는 재생하지 않습니다.

완료 직후에는 `방금 만든 보고서 열어줘`, `최근 만든 발표자료 보여줘`로 가장
최근 검증 산출물을 정확한 네이티브 앱에서 열고 맨 앞으로 가져올 수 있습니다.

`선택한 범위만 분석해서 Word 보고서와 5장짜리 PPT 만들어줘`처럼 `선택한
범위만`을 명시하면 현재 Excel의 정확한 `시트!주소`를 승인 화면에 고정하고 첫
행을 머리글로 사용해 범위 안 셀만 읽습니다. 승인 전에 선택이나 내용 문맥이
바뀌면 기존 미리보기를 폐기하며, 시트명·주소는 재사용 스킬에 저장하지 않습니다.
`이거`만 말한 기존 워크플로는 계속 현재 통합문서 전체를 뜻합니다.
