...
오른쪽 키가 중복된 경우에는 `... 주문 시트의 매출을 합계 집계하고 주문 시트의
수량을 평균 집계하고 주문 시트의 주문ID를 건수 집계하고 주문 시트의 매출을
최솟값 집계하고 주문 시트의 매출을 최댓값 집계해서 왼쪽 조인해서 ...`처럼
오른쪽 시트·열·합계/평균/건수/최솟값/최댓값을 각각 명시할 수 있습니다. 왼쪽 키가 고유한
경우에만 최대 5개 집계를 오른쪽 키별로 계산한 뒤 조인합니다. 건수는 명시한 열의
비어 있지 않은 값만 셉니다. 승인 없는 다대다·측정값 키·비숫자
합계·평균·최솟값·최댓값·행/열
...
910개 중 907개가 통과했고(unit 367·integration 337·windows 203), 실제 외부
