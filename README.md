...
완전히 같고 굵기·글자 크기·문단 정렬 중 하나만 바뀐 경우(Word·PowerPoint)는
...
그 작업만 5장으로 생성합니다. 승인된 Word 굵기·글자 크기·문단 정렬 기본값도
보고서 생성 미리보기에 표시되고, 승인 후 새 Word 보고서 전체에 적용한 다음
저장 전 COM 값과 저장 후 재열기로 검증합니다. Word·PowerPoint 일반 편집에서는
`정렬해줘`, `글자 크기 맞춰줘`, `강조 방식 맞춰줘`처럼 값 하나를 생략한 요청에만
승인된 파일/앱 기본값을 채우고, 정확한 적용값을 미리보기로 다시 승인받습니다.
...
2026-07-18 키보드·모달 접근성 보강 기준으로 전체 자동 테스트
801개 중 798개가 통과했고(unit 278·integration 317·windows 203), 실제 외부
...
통과했습니다. 10단계 승인형 다중 시트 문서 워크플로, 11단계 명시적 선호 학습과
실제 Word 서식 read-back, 12단계 개인정보 제한 진단 프로브도 모두
