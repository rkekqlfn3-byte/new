...
현재 개발 소스 버전은 `1.1.0-preview.2`이며, 보정 릴리스 준비 단계입니다.
검증된 기존 실행본은 개발 저장소 밖의
`C:\Users\PC04\OneDrive\Desktop\JARVIS_RUNTIME\current\Jarvis`에 보존돼
있습니다. 현재 소스의 새 실행본은 R5에서 만들고 R6 전체 검증을 통과한
뒤에만 이 경로를 교체합니다.
...
현재 보정 소스의 테스트는 작업 순서에 따라 R6에서 마지막으로 실행합니다.
그 전의 과거 통과 개수는 현재 소스의 결과로 재사용하지 않습니다. 외부 AI
라이브 테스트는 실제 자격 증명 사용이 승인된 경우에만 실행합니다.
