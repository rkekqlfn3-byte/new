...
class Stage10WorkflowFlowTests(unittest.TestCase):
    ...
    def test_report_only_recipe_is_approved_executed_and_reused_without_powerpoint(self):
        ...
        self.assertTrue(plan["include_report"])
        ...
        self.assertTrue(observations["include_report"])
    ...
    def test_presentation_only_recipe_is_approved_executed_and_reused_without_report(self):
        self.ppt.fail_times = 0
        conflict = self.command(
            "PPT만 만들고 Word 보고서도 만들어줘",
            "stage10-presentation-only-conflict",
        )
        self.assertFalse(conflict["success"])
        self.assertIn("하나로 다시", conflict["message"])
        self.assertEqual(0, self.analyzer.calls)
        ...
        self.assertEqual("confirmation_required", preview["status"])
        self.assertIn("보고서: 생성하지 않음", preview["message"])
        self.assertFalse(plan["include_report"])
        self.assertTrue(plan["include_presentation"])
        self.assertEqual(7, plan["slide_count"])
        self.assertEqual({"presentation"}, set(plan["output_paths"]))
        self.assertEqual(
        ...
        self.assertTrue(completed["success"], completed)
        observations = completed["data"]["observations"]
        self.assertFalse(observations["include_report"])
        self.assertEqual(1, len(observations["created_files"]))
        self.assertEqual(1, self.analyzer.calls)
        self.assertEqual(0, self.word.calls)
        self.assertEqual(0, self.hwp.calls)
        self.assertEqual(1, self.ppt.calls)
        candidate = self.workflow_skills.latest_candidate()
        self.assertFalse(candidate["template"]["include_report"])

        remember = self.command(
            "이 워크플로 기억해",
            "stage10-presentation-only-remember",
        )
        self.assertIn("Excel 분석 → PowerPoint 7장만", remember["message"])
        self.assertTrue(self.approve(remember)["success"])
        reuse = self.command(
            "지난번처럼 해줘",
            "stage10-presentation-only-reuse",
        )
        self.assertIn("Excel 분석 → PowerPoint 7장만", reuse["message"])
        reused = self.approve(reuse)

        self.assertTrue(reused["success"], reused)
        self.assertFalse(reused["data"]["observations"]["include_report"])
        self.assertEqual(2, self.analyzer.calls)
        self.assertEqual(0, self.word.calls)
        self.assertEqual(2, self.ppt.calls)
    ...
    def test_explicit_multi_aggregation_is_previewed_and_executed(self):
        ...
        self.assertIn("주문/매출 최솟값", preview["message"])
        self.assertIn("주문/매출 최댓값", preview["message"])
        ...
        self.assertIn("주문/매출 최솟값", completed["message"])
        self.assertIn("주문/매출 최댓값", completed["message"])
        self.assertIsNone(self.workflow_skills.latest_candidate())
