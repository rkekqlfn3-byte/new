...
from verification import prototype1_stage7_probe as probe


class PrototypeStage7ProbeSafetyTests(unittest.TestCase):
    def test_existing_user_word_process_skips_owned_fixture_creation(self):
        with (
            mock.patch.object(probe, "_process_ids", return_value={1234}),
            mock.patch.object(probe.multiprocessing, "get_context") as context,
        ):
            result = probe._run_isolated()
