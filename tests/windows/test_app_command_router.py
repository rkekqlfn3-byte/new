...
class AppCommandRouterTests(unittest.TestCase):
    ...
    def router(owner):
        return AppCommandRouter(
            owner.app_action_registry,
            owner.decision_engine,
            owner.preference_manager,
            owner.confirmations,
        )

    def test_prepare_selects_alias_from_injected_registry(self):
        first = FakeAdapter()
        owner = self.owner(first)
        router = self.router(owner)
    ...
    def test_execute_delegates_confirmation_without_running_adapter(self):
        ...
        owner.confirmations.queue_prepared_action.return_value = {
        ...
        result = router.execute(
            request, "session-b", "원본 명령", runtime=owner
        )

        self.assertEqual("confirmation_required", result["status"])
        self.assertEqual([], adapter.execute_calls)
        owner.confirmations.queue_prepared_action.assert_called_once_with(
            adapter.prepared,
            request,
            decision,
            "session-b",
