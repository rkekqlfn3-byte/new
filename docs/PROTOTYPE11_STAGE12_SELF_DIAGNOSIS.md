...
## 실패 책임 분류와 실행 전 복구 계약

사건 원인 범주와 별도로 다음 책임 분류를 결정적으로 계산한다.

| 책임 분류 | 다음 처리 |
| --- | --- |
| `runtime_recoverable` | 기존 기능이나 제한 탐색으로 해결 가능한지 확인 |
| `needs_input` | 한 번에 한 가지 대상·선택 정보 요청 |
| `missing_capability` | 개발 기능 목록에 익명 항목 등록 |
| `implementation_bug` | 재현 규격과 필수 회귀 테스트 등록 |
| `environment_blocked` | 앱·권한·문서 상태 해결 방법 안내 |
| `policy_blocked` | 정책을 유지한 안전 대안 안내 |
| `preference_mismatch` | 사용자 교정을 선호 후보로 연결 |
| `unknown` | 증거를 더 수집하고 자동 실행 중단 |

명시적인 앱 열기·닫기에서 대상을 찾지 못한 경우에는 공통 실행 전 복구 계약이
다음 네 조건을 모두 증명할 때만 Windows 앱 목록을 한 번 다시 확인한다.
...
대상은 요청 동사와 정규화한 후보의 해시 서명으로 비교한다. 재분석 결과의 서명이
달라지면 `RECOVERY_TARGET_CHANGED`로 기록하고 실행하지 않는다. 실행 시작 뒤의
복구 요청은 `RECOVERY_AFTER_EXECUTION_BLOCKED`로 기록하고 개발자 책임의 안전
실패로 분류한다. 결과와 이벤트에는 복구 ID·해시·불리언·횟수만 남으며 앱 이름,
경로, 사용자 문장 원문은 저장하지 않는다.

같은 계약은 명시적인 편집 요청의 준비·승인 직전 `connected_document_focus`에도
적용한다. 연결 문서가 비활성 상태일 때만 기존 세션의 창 handle을 한 번
활성화하고, foreground 성공과 동일 문서 fingerprint를 재검증한다. 상태 조회나
...
재탐색에서도 저장 문서를 찾지 못하면 최초 편집 요청에 한해
`connected_document_reopen`이 같은 fingerprint의 정확한 파일 경로를 한 번 다시
연다. 재열기 전후 파일 신원, 앱·경로, 새 창 handle, foreground, 재캡처한 문서
fingerprint가 모두 일치해야 새 미리보기를 만든다. 상태 조회·미저장 문서·기존
미리보기 승인·실행 중에는 재열지 않는다. 승인 중 닫힌 문서는 미리보기를 폐기하고
세션을 `ready`로 돌려 새 요청을 요구한다.
...
- 실행 전·대상 동일·1회 제한 복구 계약
- 복구 성공, 미발견, 대상 변경, 실행 시작 뒤 요청의 서로 다른 책임 분류
- 복구 진단에 앱 이름·경로·사용자 원문 부재
