...
명령은 항상 아래 순서로 처리한다. 이 순서를 건너뛰거나 뒤집지 않는다.

| 단계 | 이름 | 하는 일 | 비용 |
|---|---|---|---|
| 1 | 규칙 | 동사·명사 사전과 정규식으로 곧바로 작업을 정한다 | 즉시·무료·오프라인 |
| 2 | AI 해석 | 1단계가 놓친 문장을 **닫힌 작업 목록 중 하나로 번역**한다 | 1초·요금·네트워크 |
| 3 | AI 생성 | 2단계로도 안 되면 새 매크로를 만들어 제안한다 | 느림·승인 필수 |
| 4 | 학습 | 2·3단계로 성공한 표현을 규칙으로 굳혀 다음부터 1단계에서 잡는다 | — |

- **2단계에서 모델은 문서를 만지지 않는다.** 닫힌 목록에서 이름 하나를 고르고
  그 이름의 파라미터만 채운다. 목록 밖 이름, 그 작업이 받지 않는 파라미터,
  읽을 수 없는 응답, 오프라인은 모두 거부하고 1단계의 안내 문구로 되돌아간다.
- 2·3단계 결과도 미리보기·승인·문맥 지문 비교·검증·되돌리기를 그대로 거친다.
  모델이 틀리면 사용자가 미리보기에서 취소하는 것이지, 문서가 이미 바뀐 뒤가 아니다.
- 4단계는 **승인되어 실행에 성공한 표현만** 굳힌다. 실패했거나 취소된 해석은
  학습하지 않는다.
- 1단계는 절대 제거하지 않는다. 자주 쓰는 명령이 요금과 네트워크에 묶이면
  오프라인에서 앱이 죽는다.
...
| 순위 | 앱 | 기준 |
|---|---|---|
| 1 | 한글, Excel, PDF | 완결이 목표. 새 기능·검증·문서를 여기에 먼저 쓴다 |
| 2 | Word, PowerPoint | 기존 기능 유지. 1순위가 완결되기 전에는 확장하지 않는다 |

우선순위가 낮은 앱에 기능을 넣는 변경은, 1순위 앱에서 같은 기능이 이미
동작할 때만 받는다.
