...
명시적 편집 요청의 준비·승인 직전 연결 문서가 비활성 상태라면 공통 실행 전
복구 계약 아래 그 문서 창을 한 번 활성화하고 문맥을 재캡처한다. 동일 세션과 문서
fingerprint가 다시 확인된 경우에만 계속한다. 평소 상태·문맥 API 조회는 창을
활성화하지 않으며, focus 거부나 다른 문서 확인은 사용자 안내 후 차단한다.

최초 편집 요청에서 저장된 연결 문서를 열린 문서 목록에서도 찾지 못하면 파일
신원을 다시 확인하고 정확한 경로를 한 번 열 수 있다. 새 창 handle과 동일 문서
fingerprint가 확인된 뒤에만 문맥을 재캡처한다. 상태 조회, 미저장 문서, 기존
미리보기 승인에서는 문서를 자동으로 열지 않는다.
...
- 상태 조회는 focus를 바꾸지 않고, 편집 요청의 focus 복구도 동일 문서 재검증 뒤에만 성공한다.
- 닫힌 저장 문서 재열기는 최초 편집 요청·동일 파일 신원·정확 경로 1회로 제한한다.
