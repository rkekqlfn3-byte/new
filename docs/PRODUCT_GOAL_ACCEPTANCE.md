...
PDF MVP는 기존 축을 대체하거나 제품 철학을 늘리지 않고, 기존 축을 가로지르는
`pdf_document_workflow` 확장 증거 프로필로 함께 검사한다.
자동화가 통과했다는 사실을 실제 화면 읽기 프로그램, 음성 입력, 비숙련자 관찰까지
끝났다고 과장하지 않는 것이 핵심이다.

## 자동 증거 실행

기존의 최근 소유 문서 probe 결과를 사용해 전체 자동 회귀, 8개 Goal 축과 PDF
확장 증거 프로필을 판정한다. PDF 프로필은 서로 다른 120개 자연어의 의도·결과,
승인 전 쓰기 금지, 외부 AI 미호출, 제한된 시간·메모리, 소유 fixture 정리와
분할·병합·회전의 재열기 검증·원본 불변·안전 Undo를 필수로 요구한다.
앱 간 업무 축은 Word, 한글, 두 보고서 동시 생성의 세 실제 probe가
...
같은 Stage 11 probe는 발표자료 전용 구조도 검증 성공에서 경로·내용 없이
기억하고, 별도 승인 뒤 Word·한글 실행기 호출과 보고서 산출물 없이 정확히
5장인 PowerPoint 하나만 새 경로에 재생했다는
`presentation_only_workflow_skill_actual_replay_verified`도 남겨야 한다.
